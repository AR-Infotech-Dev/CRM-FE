...
} from "@modules/company-master/utils/companyMaster.utils";

export const useCompanySettingForm = ({ isOpen = true, onClose, selectedCompany, onAfterSave, loggedInCompanyId, }) => {
  ...
  const [formData, setFormData] = useState(companySettingSchema.form.initialValues);
  const [errors, setErrors] = useState({});
  const selectedCompanyId = getCompanyIdentifier(selectedCompany);
  const companyId = selectedCompanyId || loggedInCompanyId || null;
  ...
  useEffect(() => {
    const fetchCompanyDetails = async () => {
      ...
      try {
        ...
        const companyData = res?.data?.data || res?.data || res;
        const normalizedData = normalizeCompanyData(companyData);
  ...
  const validatePayload = (payload) => {
    const result = companySettingSchema.validationSchema.safeParse(payload);
    if (result.success) {
      setErrors({});
      return true;
    }
